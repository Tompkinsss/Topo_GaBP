...
DispatcherThread::DispatcherThread(
    std::unique_ptr<class Dispatcher> dispatcher, size_t host_rank, size_t process_per_host, size_t workers_per_host)
    : dispatcher_(std::move(dispatcher)),
      host_rank_(host_rank), process_per_host_(process_per_host), workers_per_host_(workers_per_host) {
    // start thread
    thread_ = std::thread(&DispatcherThread::Work, this);
}
...
void DispatcherThread::Work() {
    ...
    host_rank_ = host_rank_ % process_per_host_;

    //int core_num = ((host_rank_ % numa_cores) + 1) * numa_cpus / numa_cores - host_rank_ / numa_cores - 1;
    
    int core_num = (host_rank_+1)*(workers_per_host_+1)-1;
