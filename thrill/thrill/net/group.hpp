...
public:
    ...
    template <typename T, typename BinarySumOp = std::plus<T> >
    T InterMap(T& value, BinarySumOp sum_op = BinarySumOp());
...
/*[[[perl
    ...
    ["long long", "LongLong"], ["unsigned long long", "UnsignedLongLong"])
  {
    ...
    virtual int InterMapPlusInt(int& value);
...
template <>
inline int Group::InterMap(int& value,std::plus<int>){
    return InterMapPlusInt(value);
}
