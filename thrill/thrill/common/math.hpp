...
public:
    ...
    Range Partition(size_t i, size_t parts) const {
        ...
        //if(i>0){
        //return Range(CalculateBeginOfPart(i, parts)-1,
        //             CalculateBeginOfPart(i + 1, parts)-1);
        //}else{
...
static inline Range CalculateLocalRange(size_t rows, size_t columns, size_t p, size_t i){
    return Range(0,rows * columns).Partition(i,p);
}
